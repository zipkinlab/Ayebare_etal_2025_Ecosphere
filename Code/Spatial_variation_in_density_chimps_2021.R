...
#------------------------------------------------------------------------------------------------------------------------------------------##
#### inputs "d2021_chimps_hds.tif" and "d2021_chimps_mhds.tif" are obtained after running the script: Population_status_and_change_spatial.R
#------------------------------------------------------------------------------------------------------------------------------------------##


# Read in rasters
chimp_hds <- terra::rast("d2021_chimps_hds.tif")
chimp_hds

chimp_mhds <- terra::rast("d2021_chimps_mhds.tif")
summary(chimp_mhds)
...
ggplot2::ggplot() + 
  ...
  ggplot2::scale_fill_gradientn(
    ...
    limits  = c(0.07, 3.4),
    values  = scales::rescale(c(0.07, 1.5, 3.4), from = c(0.07, 3.4)), 
    breaks = c( 0.07, 1.5, 3.4),
    na.value = NA) 
...
# save map
ggplot2::ggsave(
  filename = "Figure5_chimp_density_2021.jpg", 
  width = 15, 
  height = 10, 
  units = "in", 
  dpi = 400
