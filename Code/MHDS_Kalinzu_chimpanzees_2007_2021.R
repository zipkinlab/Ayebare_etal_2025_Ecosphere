...
### Script: MHDS_Kalinzu_chimpanzees_2007_2021.R
...
### filter kalinzu observations 
V23.2021<- filter(V23.2021, Tr > 36)
head(V23.2021)
...
### filter kalinzu observations 
V123.2021 <- filter(V123.2021, Tr > 36)
...
###------------------------------------------------------------------------------------##
Kalinzu.V1.2007  <- sum(N1.2007[])
Kalinzu.V1.2021  <- sum(N1.2021[])
Kalinzu.V23.2007 <- sum(N23.2007[])
Kalinzu.V23.2021 <- sum(N23.2021[])
...
## 1.5026 and 1.5182: represent the total area surveyed in sqkm
##---------------------------------------------------------------------------------------------------------------##

DKalinzu23.2007  <- Kalinzu.V23.2007 /(1.09*50* 1.5026)
DKalinzu23.2021  <- Kalinzu.V23.2021 /(1.09*49* 1.5182)

##---------------------------------------------------------------------------------------------------------------##
#### posterior distribution of a change in population density between two survey periods (i.e., 2007 and 2021)
##---------------------------------------------------------------------------------------------------------------##
DKalinzu_pop_change = DKalinzu23.2021  - DKalinzu23.2007 

##---------------------------##
## Nest decay rate
##---------------------------##
No.days.V1.2007 <- Kalinzu.V1.2007   / (DKalinzu23.2007 * 1.09*1.5026)
No.days.V1.2021 <- Kalinzu.V1.2021  / (DKalinzu23.2021 * 1.09*1.5182)

##--------------------------------------------------##
# Total number of expected signs in the surveyed area
##---------------------------------------------------##

Kalinzu.lambda.V1.07 <- sum(lambda1.2007[])
Kalinzu.lambda.V23.07 <- sum(lambda23.2007[])
Kalinzu.lambda.V1.21 <- sum(lambda1.2021[])
Kalinzu.lambda.V23.21 <- sum(lambda23.2021[])

}
",fill=TRUE, file = "Kalinzu.pop_spatial_temporal.txt")
...
inits <- function(){list( beta0.V1.2007 =runif(1, 0, 1),beta0.V23.2007=runif(1, 0, 1), beta0.V1.2021 =runif(1, 0, 1),beta0.V23.2021=runif(1, 0, 1),
                          beta1.07 =runif(1, 0, 1),beta2.07 =runif(1, 0, 1),beta1.21=runif(1, 0, 1), beta2.21 =runif(1, 0, 1),N23.2007 = Nst23.2007, 
                          N23.2021 = Nst23.2021, N1.2007 = Nst1.2007, N1.2021 = Nst1.2021,  sigma1.2007=runif(1,1,10),sigma1.2021=runif(1,1,10),
...
params <- c( "beta0.V1.2007", "beta0.V23.2007", " beta0.V1.2021", "beta0.V23.2021","N1.2007", "No.days.V1.2007","No.days.V1.2021",
             ...
             "lambda23.2007", "lambda23.2021", "Kalinzu.lambda.V1.07","Kalinzu.V1.2007", "Kalinzu.lambda.V23.07","Kalinzu.V23.2007", 
             "Kalinzu.lambda.V1.21","Kalinzu.V1.2021", "Kalinzu.lambda.V23.21", "Kalinzu.V23.2021", "DKalinzu23.2007","DKalinzu23.2021",
             "DKalinzu_pop_change", "pcap23.2007","pcap23.2021", "sigma23.2007","sigma23.2021")
...
Kal_2007_2021_chimps <- jags(data_C2007n2021, inits, params, "Kalinzu.pop_spatial_temporal.txt", n.thin=nt,
                                  n.chains=nc, n.burnin=nb, n.iter=ni,  parallel = T)


Kal_2007_2021_chimps


### save output
save(Kal_2007_2021_chimps , file = "Kal_2007_2021_chimps_mhds.RData")


### Write output as a .csv

Kal_2007_2021.chimps <- Kal_2007_2021_chimps$summary

write.csv(Kal_2007_2021.chimps, "Kal_2007_2021_ouput_chimps_q.csv")
...
#### Kalinzu Forest Reserve
### Load 
load(".../Kal_2007_2021_chimps_mhds.RData")
...
hist(Kal_2007_2021_chimps$sims.list$DKalinzu_pop_change)


### Estimate the probability that the population has decreased
decrease <- (sum(Kal_2007_2021_chimps$sims.list$DKalinzu_pop_change  < 0)/15000)
decrease

### Estimate the probability that the population has increased
increase <- (sum(Kal_2007_2021_chimps$sims.list$DKalinzu_pop_change  > 0)/15000)
increase
...
five.pct.decrease <- -1 * (Kal_2007_2021_chimps$mean$DKalinzu23.2007 * 5/100)
five.pct.decrease
### Probability that the population has decreased by 5 percent
prob.decrease.5pct <- (sum(Kal_2007_2021_chimps$sims.list$DKalinzu_pop_change < five.pct.decrease)/15000)
prob.decrease.5pct
...
####------------------------------##
ten.pct.decrease <- -1 * (Kal_2007_2021_chimps$mean$DKalinzu23.2007 * 10/100)
ten.pct.decrease
### Probability that the population has decreased by 10 percent
prob.decrease.10pct <- (sum(Kal_2007_2021_chimps$sims.list$DKalinzu_pop_change < ten.pct.decrease)/15000)
prob.decrease.10pct
...
####------------------------------##
twenty.pct.decrease <- -1 * (Kal_2007_2021_chimps$mean$DKalinzu23.2007 * 20/100)
twenty.pct.decrease
### Probability that the population has decreased by 20 percent
prob.decrease.20pct <- (sum(Kal_2007_2021_chimps$sims.list$DKalinzu_pop_change < twenty.pct.decrease)/15000)
prob.decrease.20pct
...
####------------------------------##
thirty.pct.decrease <- -1 * (Kal_2007_2021_chimps$mean$DKalinzu23.2007 * 30/100)
thirty.pct.decrease
### Probability that the population has decreased by 30 percent
prob.decrease.30pct <- (sum(Kal_2007_2021_chimps$sims.list$DKalinzu_pop_change < thirty.pct.decrease)/15000)
prob.decrease.30pct
...
####------------------------------##
forty.pct.decrease <- -1 * (Kal_2007_2021_chimps$mean$DKalinzu23.2007 * 40/100)
forty.pct.decrease
### Probability that the population has decreased by 40 percent
prob.decrease.40pct <- (sum(Kal_2007_2021_chimps$sims.list$DKalinzu_pop_change < forty.pct.decrease)/15000)
prob.decrease.40pct
...
####------------------------------##
fifty.pct.decrease <- -1 * (Kal_2007_2021_chimps$mean$DKalinzu23.2007 * 50/100)
fifty.pct.decrease
### Probability that the population has decreased by 50 percent
prob.decrease.50pct <- (sum(Kal_2007_2021_chimps$sims.list$DKalinzu_pop_change < fifty.pct.decrease)/15000)
prob.decrease.50pct
