...
### Script: CV_Maramagambo_elephants_2021_HDS.R
...
#--------------------------------------#
obs_Rd23_cv <- uncount(Rd23_obs_cv, eldung)
dim(obs_Rd23_cv )
head(obs_Rd23_cv )
...
#---------------------------------------#
eledung_cv <- as_tibble(V123)
head(eledung_cv)
dim(eledung_cv)


#-------------------------------------------------------#
## Number of elephant dung detected per transect- Visit 2 &3
#-------------------------------------------------------#
V23.2021_cv <- eledung_cv$V23

#-------------------------------------------------------#
## Replace observed elephant dung at transect one with - NA
#-------------------------------------------------------#
V23.2021_cv <- eledung_cv$V23
V23.2021_tr1 <- replace(V23.2021_cv, 1, NA)
...
#--------------------------------#
delta23_cv <- 1  #  max distance meters

#--------------------------------#
## perpendicular distance meters
#--------------------------------#
B23_cv <- 5  #  #  # Effective strip width 

#Distance class midpoint ID
midpt23_cv <-  c(0.5,1.5,2.5,3.5,4.5)

#--------------------------------------#
## Total number of Elephant dungs observed
## during visits 2 & 3
#--------------------------------------#
...
data_V23.tr1 <- list(nsites=nsites, nind23_cv= nind23_cv, B23_cv =B23_cv, db23_cv= db23_cv, midpt23_cv =midpt23_cv,
                     delta23_cv =delta23_cv,dclass23_cv =dclass23_cv, V23.2021_tr1 = V23.2021_tr1,
                     A = A, elev = elev) 
...
# Construct cell probabilities 
  ...
  for(s in 1:nsites){
     V23.2021_tr1[s] ~ dbin(pcap23_cv, N23_cv[s])   # Part 2 of HM
...
Expected_no_dung_tr1 <- lambda23_cv[1]
...
",fill=TRUE, file = "Eles_2021_hds_tr1.txt")




# Initial values
Nst23_cv <- V23.2021_cv + 1

inits <- function(){list(beta0_cv=runif(1, 0, 1),beta1_cv=runif(1, 0, 1), beta2_cv=runif(1, 0, 1),  
                         N23_cv=Nst23_cv, sigma23_cv=runif(1,1,5))}
...
Eles_2021_hds.tr1 <- jags(data_V23.tr1, inits, params, "Eles_2021_hds_tr1.txt", n.thin=nt,
...
save(Eles_2021_hds.tr1, file = "Eles_2021_hds.tr1.RData")


### Write output as a .csv

Eles_2021_hds_tr1 <- Eles_2021_hds.tr1$summary

write.csv(Eles_2021_hds_tr1, "Eles_2021_hds_tr1_output_CV.csv")
